...
const rows = ref<CustomerManageItem[]>([])
const total = ref(0)
const page = ref(1)
...
async function loadTable(): Promise<void> {
  ...
  try {
    const data = await queryCustomers({
      customerNickname: query.customerNickname || undefined,
...
<template>
  <el-container class="page-root">
    <el-aside :width="layoutStore.collapsed ? '72px' : '220px'" class="sider">
      ...
      <el-menu :collapse="layoutStore.collapsed" default-active="4-5" class="menu" @select="onMenuSelect">
        ...
        <el-card class="page-card">
          <el-form :model="query" inline>
            <el-form-item label="客户昵称">
              <el-input v-model="query.customerNickname" placeholder="请输入" clearable />
            </el-form-item>
            <el-form-item label="客户手机号">
              <el-input v-model="query.customerPhone" placeholder="请输入" clearable />
            </el-form-item>
            <el-form-item>
          ...
          <el-table :data="rows" border stripe v-loading="loading" table-layout="auto">
            ...
            <el-table-column prop="customerPhone" label="客户手机号" min-width="150" />
            <el-table-column prop="signed" label="是否签约" min-width="100" />
            <el-table-column prop="orderTrackCount" label="跟踪下单次数（次）" min-width="140" />
            <el-table-column prop="bindElderNames" label="绑定老人姓名" min-width="180" />
            <el-table-column prop="firstLoginTime" label="首次登录时间" min-width="180" />
          </el-table>
