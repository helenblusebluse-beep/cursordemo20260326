...
async function loadTable(): Promise<void> {
  loading.value = true
  try {
    const data = await queryCheckins({
...
function goApply(): void {
  void router.push('/checkins/apply')
}

function goDetail(row: CheckinItem): void {
  void router.push(`/checkins/${row.id}`)
}
...
<template>
  <el-container class="page-root">
    <el-aside :width="layoutStore.collapsed ? '72px' : '220px'" class="sider">
      ...
      <el-menu :collapse="layoutStore.collapsed" default-active="3-2" class="menu" @select="onMenuSelect">
    ...
    <el-container>
      ...
      <el-main class="main">
        <el-card class="page-card">
          ...
          <div class="toolbar">
            <el-button type="primary" @click="goApply">发起入住申请</el-button>
          </div>
          ...
          <el-table :data="rows" border stripe v-loading="loading" table-layout="auto">
            ...
            <el-table-column prop="idCard" label="老人身份证号" min-width="160" />
            <el-table-column prop="roomNo" label="入住床位" min-width="90" />
            <el-table-column prop="careLevel" label="护理等级" min-width="120" />
            <el-table-column prop="checkinPeriod" label="入住期限" min-width="170" />
