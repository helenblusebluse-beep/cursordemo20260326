...
import { createPrepayRecharge, queryPrepayElderOptions, queryPrepayRecharges } from '../api/prepay'
import type { PrepayElderOption, PrepayRechargeItem } from '../types/prepay'

const router = useRouter()
const layoutStore = useLayoutStore()
const loading = ref(false)
const rows = ref<PrepayRechargeItem[]>([])
const total = ref(0)
const page = ref(1)
...
const elderOptions = ref<PrepayElderOption[]>([])
const form = reactive({
...
async function loadTable(): Promise<void> {
  loading.value = true
  try {
    const data = await queryPrepayRecharges({
...
async function loadElders(): Promise<void> {
  elderOptions.value = await queryPrepayElderOptions()
}
...
const beforeUpload: UploadProps['beforeUpload'] = (rawFile) => {
  ...
  const okType = ['png', 'jpg', 'jpeg'].includes(ext)
  if (!okType) {
    ElMessage.warning('仅支持上传PNG/JPG/JPEG类型图片')
    return false
  }
  const okSize = rawFile.size / 1024 / 1024 <= 2
  if (!okSize) {
...
async function doUpload(option: UploadRequestOptions): Promise<void> {
  const fd = new FormData()
  fd.append('file', option.file)
  try {
    const res = await axios.post('/api/files/upload-image', fd, {
      headers: { 'Content-Type': 'multipart/form-data' },
      timeout: 15000,
    })
    if (res.data?.code !== 1) throw new Error(res.data?.msg || '上传失败')
    form.voucherFileName = res.data.data.fileName
    form.voucherFileUrl = res.data.data.fileUrl
...
async function submitCreate(): Promise<void> {
  ...
  if (!form.voucherFileUrl || !form.voucherFileName) {
    ElMessage.warning('请上传充值凭证')
    return
  }
  if (!form.rechargeRemark.trim()) {
    ElMessage.warning('请输入充值备注')
    return
  }
  if (form.rechargeRemark.trim().length > 50) {
    ElMessage.warning('充值备注最多50个字符')
    return
  }
  saving.value = true
  try {
    await createPrepayRecharge({
      elderId: form.elderId,
      rechargeMethod: form.rechargeMethod,
      rechargeAmount: form.rechargeAmount,
      voucherFileName: form.voucherFileName,
      voucherFileUrl: form.voucherFileUrl,
      rechargeRemark: form.rechargeRemark.trim(),
    })
...
<template>
  <el-container class="page-root">
    <el-aside :width="layoutStore.collapsed ? '72px' : '220px'" class="sider">
      ...
      <el-menu :collapse="layoutStore.collapsed" default-active="8-2" class="menu" @select="onMenuSelect">
    ...
    <el-container>
      ...
      <el-main class="main">
        <el-card>
          ...
          <div class="toolbar"><el-button type="primary" @click="openCreate">充值</el-button></div>
          <el-table :data="rows" border stripe v-loading="loading">
            <el-table-column type="index" label="序号" width="60" />
            <el-table-column prop="rechargeNo" label="充值流水号" min-width="180" />
            <el-table-column prop="elderName" label="老人姓名" min-width="90" />
            <el-table-column prop="elderIdNo" label="老人身份证号" min-width="170" />
            <el-table-column prop="rechargeMethod" label="充值方式" min-width="90" />
            <el-table-column prop="rechargeAmount" label="充值金额（元）" min-width="110" />
            <el-table-column label="充值凭证" min-width="90">
              <template #default="{ row }"><el-image :src="row.voucherFileUrl" style="width:40px;height:40px" fit="cover" /></template>
            </el-table-column>
            <el-table-column prop="rechargeRemark" label="充值备注" min-width="180" />
  ...
  <el-dialog v-model="dialogVisible" title="上传充值凭证" width="680px" destroy-on-close>
    <el-form label-width="100px">
      <el-form-item label="* 老人姓名">
        <el-select v-model="form.elderId" placeholder="请选择" style="width:100%">
          <el-option v-for="x in elderOptions" :key="x.elderId" :label="`${x.elderName}（${x.bedNo}）`" :value="x.elderId" />
        </el-select>
      </el-form-item>
      <el-form-item label="* 充值方式">
        <el-select v-model="form.rechargeMethod" placeholder="请选择" style="width:100%">
      ...
      <el-form-item label="* 充值金额">
        <el-input-number v-model="form.rechargeAmount" :min="0.01" :precision="2" :step="1" />
      </el-form-item>
      <el-form-item label="* 充值凭证">
        <el-upload :show-file-list="false" :before-upload="beforeUpload" :http-request="doUpload">
          <div class="upload-box" v-if="!form.voucherFileUrl">点击上传图片</div>
          <el-image v-else :src="form.voucherFileUrl" style="width:100px;height:100px;border:1px solid #dcdfe6" fit="cover" />
        </el-upload>
        <div class="upload-tip">图片大小不超过2M，仅支持上传PNG JPG JPEG类型图片</div>
      </el-form-item>
      <el-form-item label="* 充值备注">
        <el-input v-model="form.rechargeRemark" maxlength="50" placeholder="请输入" />
      </el-form-item>
    </el-form>
    <template #footer>
...
.upload-box { width: 100px; height: 100px; border: 1px dashed #dcdfe6; display: flex; align-items: center; justify-content: center; color: #909399; }
