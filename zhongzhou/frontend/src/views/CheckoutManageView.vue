...
import { queryCheckouts } from '../api/checkout'
import type { CheckoutItem } from '../types/checkout'

const router = useRouter()
const layoutStore = useLayoutStore()
const loading = ref(false)
const rows = ref<CheckoutItem[]>([])
const total = ref(0)
const page = ref(1)
...
async function loadTable(): Promise<void> {
  loading.value = true
  try {
    const data = await queryCheckouts({
...
function onApply(): void {
  void router.push('/checkouts/apply')
}

function goDetail(row: CheckoutItem): void {
  void router.push(`/checkouts/${row.id}`)
}
...
<template>
  <el-container class="page-root">
    <el-aside :width="layoutStore.collapsed ? '72px' : '220px'" class="sider">
      ...
      <el-menu :collapse="layoutStore.collapsed" default-active="3-3" class="menu" @select="onMenuSelect">
      ...
      <el-main class="main">
        <el-card class="page-card">
          <el-form :model="query" inline>
            ...
            <el-form-item label="老人身份证号"><el-input v-model="query.idCard" placeholder="请输入" clearable /></el-form-item>
            <el-form-item>
          ...
          <div class="toolbar">
            <el-button type="primary" @click="onApply">发起退住申请</el-button>
          </div>
          ...
          <el-table :data="rows" border stripe v-loading="loading" table-layout="auto">
            ...
            <el-table-column prop="idCard" label="老人身份证号" min-width="180" />
            <el-table-column prop="checkoutDate" label="退住日期" min-width="130" />
