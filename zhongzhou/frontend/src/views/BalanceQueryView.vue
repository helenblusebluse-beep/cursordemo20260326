...
import { queryBalanceQueries } from '../api/balance'
import type { BalanceQueryItem } from '../types/balance'

const router = useRouter()
const layoutStore = useLayoutStore()
const loading = ref(false)
const rows = ref<BalanceQueryItem[]>([])
const total = ref(0)
const page = ref(1)
...
async function loadTable(): Promise<void> {
  loading.value = true
  try {
    const data = await queryBalanceQueries({
...
<template>
  <el-container class="page-root">
    <el-aside :width="layoutStore.collapsed ? '72px' : '220px'" class="sider">
      ...
      <el-menu :collapse="layoutStore.collapsed" default-active="8-3" class="menu" @select="onMenuSelect">
      ...
      <el-main class="main">
        <el-card>
          ...
          <el-table :data="rows" border stripe v-loading="loading">
            ...
            <el-table-column prop="accountBalance" label="预缴款余额（元）" min-width="150" />
            <el-table-column prop="depositBalance" label="押金余额（元）" min-width="130" />
            <el-table-column prop="changedTime" label="变动时间" min-width="170" />
          </el-table>
